...
function ptsb_to_utf8($s) {
    ...
    if (function_exists('seems_utf8') && seems_utf8($s)) {
        return $s;
    }
...
function ptsb_hsize_compact($bytes) {
    ...
    $mb = 1048576;
    if ($b >= $tb) {
        return number_format_i18n($b / $tb, 1) . ' TB';
    }
    if ($b >= $gb) {
        return number_format_i18n($b / $gb, 1) . ' GB';
    }
